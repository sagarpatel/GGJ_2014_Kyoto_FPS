...
function Start () {
	isHungry = false;
	targetPosition = GameObject.Find("hamburger").transform.position;
	if(GetComponent("CharacterController")==null)
		gameObject.AddComponent(typeof(CharacterController));
	characterController = GetComponent(CharacterController);
	characterController.radius = 0.2f;
	characterController.center = Vector3 (0, 0, -0.4);
	while(true){
		yield Idle();
...
function Idle(){
	...
	while(true){
		...
		startPosition = this.transform.position;
			...
			this.transform.position  = Vector3.Lerp(startPosition, targetPosition, lerpVelocity);
